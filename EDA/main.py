...
sns.countplot(data=data, y='Training Program Name', hue='Training Outcome',
              order=data['Training Program Name'].value_counts().index)
plt.title('Training Outcome Distribution by Program')
plt.show()

# cost comparison between training type 
sns.boxplot(data=data, x='Training Type', y='Training Cost')
plt.title('Training Cost by Type (Internal vs. External)')
plt.show()
